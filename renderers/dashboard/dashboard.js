...
windowLoaded.then(() => { myClassSave = new ClassSave('dashboardData'); });
...
let dockedLocation = null;
let music = null
let save = 1;
//! TIMEZONE STUFF
let currentTime = new Date();
currentTime.toISOString()
document.getElementById('korea').innerText = timeConvert(currentTime,-9)


//!First time run for Gen Info
const runSizeDivs = [
  'genInfo',
  'shipInfo',
  'locationInfo'
...
function locationDataF(locationData) {
  ...
  try {
    ...
    console.log("dockedStatus", dockedStatus)
    if (dockedStatus != "Docked") { 
      document.getElementById('StationName').classList.add("w3-hide"); 
      document.getElementById('StationType').classList.add("w3-hide") 
    }
    ...
    if (locationData['DistFromStarLS']) {
      document.getElementById('DistFromStarLS').innerText = "Distance from Star:\n" + locationData['DistFromStarLS'].toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) + " LS"
    }
...
//! SUPERCRUISEEXIT DATA
let SupercruiseExitData = getEventFromStore('SupercruiseExit')
if (SupercruiseExitData) { SupercruiseExitDataF(SupercruiseExitData) }
else { console.log("SupercruiseExit, No Event Data") }
function SupercruiseExitDataF(FSDJumpData) {
  try {
    // if (SupercruiseExitData['StationName']) { 
    //   if (document.getElementById('StationName').classList.contains("w3-hide")) { document.getElementById('StationName').classList.remove("w3-hide") }
    //   const EE = document.getElementById('StationName'); 
    //   EE.innerText = "Station Name:\n" + SupercruiseExitData['StationName'].toUpperCase()
    // }
    // // else { document.getElementById('StationName').classList.add("w3-hide") }
    // if (SupercruiseExitData['StationType']) { 
    //   if (document.getElementById('StationType').classList.contains("w3-hide")) { document.getElementById('StationType').classList.remove("w3-hide") }
    //   const EE = document.getElementById('StationType'); 
    //   EE.innerText = "Station Type:\n" + SupercruiseExitData['StationType']
    // }
    // else { document.getElementById('StationType').classList.add("w3-hide") }
    // document.getElementById('StarSystem').innerText = "Star System:\n" + SupercruiseExitData['StarSystem']
    // document.getElementById('Body').innerText = "Body:\n" + "Star"
    // document.getElementById('Population').innerText = "Population:\n" + SupercruiseExitData['Population'].toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })
    // document.getElementById('SystemAllegiance').innerText = "System Allegiance:\n" + SupercruiseExitData['SystemAllegiance']
    // document.getElementById('SystemEconomy_Localised').innerText = "Economy:\n" + SupercruiseExitData['SystemEconomy_Localised']
    // document.getElementById('SystemGovernment_Localised').innerText = "Government:\n" + SupercruiseExitData['SystemGovernment_Localised']
    // document.getElementById('SystemSecurity_Localised').innerText = "Security:\n" + SupercruiseExitData['SystemSecurity_Localised']
  }
  catch(e){console.log(e);}
}
ipcRenderer.on('SupercruiseExit', (SupercruiseExitData) => { if (SupercruiseExitData) { SupercruiseExitDataF(SupercruiseExitData); } else { console.log("SupercruiseExit, No Event Data") } });
...
function dockedDataF(dockedData) {
  try {
    ...
    document.getElementById('Body').innerText = "Body:\n"
    document.getElementById('DistFromStarLS').innerText = "Distance from Star:\n" + dockedData.DistFromStarLS.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) + " LS"
...
//! CARRIERJUMP DATA
let CarrierJumpData = getEventFromStore('CarrierJump'); 
if (CarrierJumpData) { CarrierJumpDataF(CarrierJumpData); }
else { console.log("CarrierJump, No Event Data") }
function CarrierJumpDataF(CarrierJumpData) {
  if (dockedData.StationName == 'FleetCarrier') {
    console.log("Carrier jumped and I'm at a fleet carrier.")
    document.getElementById('StarSystem').innerText = "StarSystem: \n" + CarrierJumpData.StarSystem;
  }
  document.getElementById('CarrierLocation').innerText = "Carrier Location: \n" + CarrierJumpData['Body'];
}
ipcRenderer.on('CarrierJump', (CarrierJumpData) => { if (CarrierJumpData) { CarrierJumpDataF(CarrierJumpData); } else { console.log("CarrierJump, No Event Data") } });
