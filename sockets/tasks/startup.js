try {
    ...
    function transmitter(data,callback) {
        ...
        socket.emit('eventTransmit',data, (response) => {
            ...
            if (watcherConsoleDisplay(data.event)) { 
                console.log(`[SOCKET SERVER - STARTUP - '${data.event}']`.yellow)
    ...
    const startUp = {
        ...
        Commander: function(data,callback) { 
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        Missions: function(data,callback) {
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        Statistics: function(data,callback) {
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        Materials: function(data,callback) {
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        Loadout: function(data,callback) {
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
    }
