try {
    ...
    function transmitter(data,callback) {
        ...
        socket.emit('eventTransmit',data, (response) => {
            ...
            if (watcherConsoleDisplay(data.event)) { 
                console.log(`[SOCKET SERVER - FLEET CARRIERS - '${data.event}']`.yellow)
    ...
    const otherEvents = {
        ...
        CarrierJumpRequest: function(data,callback) { 
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        CarrierJump: function(data,callback) { 
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        CarrierJumpCancelled: function(data,callback) { 
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
    }
