try {
    ...
    function transmitter(data,callback) {
        ...
        socket.emit('eventTransmit',data, (response) => {
            ...
            if (watcherConsoleDisplay(data.event)) { 
                console.log(`[SOCKET SERVER - STATION SERVICES - '${data.event}']`.yellow)
    ...
    const otherEvents = {
        ...
        CommunityGoal: function(data,callback) { 
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        CommunityGoalJoin: function(data,callback) {
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        EngineerProgress: function(data,callback) {
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        },
        ShipyardSwap: function(data,callback) {
            try { data = {...data}; transmitter(data,callback); }
            catch(error) { errorHandler(error,error.name) }
        }
    }
